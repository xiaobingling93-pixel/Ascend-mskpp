...
import os
import unittest
from mskpp import vrsqrt, Tensor, Chip
...
class TestUtilsMethods(TestBase):
    ...
    def test_vrsqrt_fp16_expect_trace_successful_create(self):
        with Chip("Ascend910B1") as chip:
            chip.enable_trace()
            in_x = Tensor("GM", "FP16", [32, 64], format="ND")
            in_y = Tensor("GM", "FP16", [32, 64], format="ND")
            out_y = my_vrsqrt(in_x)
            in_y.load(out_y)
            trace_file = os.path.join(chip.output_dir, self.TRACE_FILE)
        self.assertTrue(os.path.exists(trace_file))
        self.clean()

    def test_vrsqrt_fp32_expect_trace_successful_create(self):
        with Chip("Ascend910B1") as chip:
            chip.enable_trace()
            in_x = Tensor("GM", "FP32", [32, 64], format="ND")
            in_y = Tensor("GM", "FP32", [32, 64], format="ND")
            out_y = my_vrsqrt(in_x)
            in_y.load(out_y)
            trace_file = os.path.join(chip.output_dir, self.TRACE_FILE)
        self.assertTrue(os.path.exists(trace_file))
