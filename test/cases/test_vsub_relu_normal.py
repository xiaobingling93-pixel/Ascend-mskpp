...
import os
import unittest
from mskpp import vsubrelu, Tensor, Chip
...
def my_vsub_relu(gm_x, gm_y, gm_z):
    x = Tensor(UB_MEM)
    y = Tensor(UB_MEM)
    z = Tensor(UB_MEM)
...
class TestUtilsMethods(TestBase):
    ...
    def test_vsubrelu_normal_fp16(self):
    ...
    def test_vsubrelu_normal_int16(self):
        with Chip("Ascend910B1") as chip:
            chip.enable_trace()
            for _ in range(1):
