...
import os
import unittest
from mskpp import vmulconv, Tensor, Chip
...
def my_vmul_conv(gm_x, gm_y, gm_z, dtype):
    x = Tensor(UB_MEM)
    y = Tensor(UB_MEM)
    z = Tensor(UB_MEM)
...
class TestUtilsMethods(TestBase):
    ...
    def test_vmulconv_f162s8_normal(self):
        with Chip("Ascend910B1") as chip:
            ...
            for _ in range(1):
                ...
                my_vmul_conv(in_x, in_y, in_z, INT_8_TYPE)
            trace_file = os.path.join(chip.output_dir, self.TRACE_FILE)
        self.assertTrue(os.path.exists(trace_file))
        self.clean()

    def test_vmulconv_f162u8_normal(self):
        with Chip("Ascend910B1") as chip:
            chip.enable_trace()
            for _ in range(1):
                in_x = Tensor(GM_MEM, FP_16_TYPE, [8, 2048], format=FORMAT_TYPE)
                in_y = Tensor(GM_MEM, FP_16_TYPE, [8, 2048], format=FORMAT_TYPE)
                in_z = Tensor(GM_MEM, UINT_8_TYPE, [8, 2048], format=FORMAT_TYPE)
                my_vmul_conv(in_x, in_y, in_z, UINT_8_TYPE)
            trace_file = os.path.join(chip.output_dir, self.TRACE_FILE)
        self.assertTrue(os.path.exists(trace_file))
        self.clean()
