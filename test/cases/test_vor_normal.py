...
import os
import unittest
from mskpp import vor, Tensor, Chip
...
class TestUtilsMethods(TestBase):
    ...
    def test_vor_int16_normal(self):
        with Chip("Ascend910B1") as chip:
            ...
            for _ in range(1):
                ...
                out_z = my_vor(in_x, in_y, in_z)
                in_z.load(out_z)
            trace_file = os.path.join(chip.output_dir, self.TRACE_FILE)
        self.assertTrue(os.path.exists(trace_file))
        self.clean()

    def test_vor_uint16_normal(self):
        with Chip("Ascend910B1") as chip:
            chip.enable_trace()
            for _ in range(1):
                in_x = Tensor(GM_MEM, UINT_16_TYPE, [8, 2048], format=FORMAT_TYPE)
                in_y = Tensor(GM_MEM, UINT_16_TYPE, [8, 2048], format=FORMAT_TYPE)
                in_z = Tensor(GM_MEM, UINT_16_TYPE, [8, 2048], format=FORMAT_TYPE)
                out_z = my_vor(in_x, in_y, in_z)
                in_z.load(out_z)
            trace_file = os.path.join(chip.output_dir, self.TRACE_FILE)
        self.assertTrue(os.path.exists(trace_file))
